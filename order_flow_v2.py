    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                buy_volume = sum(float(t["vol"]) for t in trades if t["side"] == 1) / 1000
                sell_volume = sum(float(t["vol"]) for t in trades if t["side"] == 2) / 1000
                total = buy_volume + sell_volume
...
def get_aggregated_orderflow():
    """Aggregate order flow from all 5 exchanges"""
