...
def scan_for_signals(timeframe: str = "15m", trade_mode: str = "day_trade") -> Optional[Signal]:
    ...
    try:
        ...
        # Anchor fib to the SWEPT LEVEL (true structural high/low), not the
        # local BOS fractal — fixes undersized fib ranges on all timeframes.
        sh = level_price   if fib_dir == "short" else bos.bos_price
        sl = bos.bos_price if fib_dir == "short" else level_price
