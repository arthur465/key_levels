...
def get_mexc_trades():
    """MEXC trades - FIXED with better error handling"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                if not trades:
                    print(f"   ❌ [MEXC] No trades data")
                    return None
                
                # Try different field names - MEXC API can vary
                buy_volume = 0
                sell_volume = 0
                
                for t in trades:
                    try:
                        # Try "vol" or "volume" or "qty"
                        vol = float(t.get("vol", t.get("volume", t.get("qty", 0))))
                        
                        # Side can be 1/2 or "buy"/"sell"
                        side = t.get("side", t.get("direction", 0))
                        
                        if side == 1 or side == "buy":
                            buy_volume += vol
                        elif side == 2 or side == "sell":
                            sell_volume += vol
                ...
                if total == 0:
                    print(f"   ❌ [MEXC] No valid trade data")
                    return None
