...
def calculate_orderflow_features(current_orderflow, history):
    """Calculate enhanced orderflow metrics"""
    ...
    return {
        ...
        'spread_pct': current_orderflow.get('spread_pct', 0),
    ...
    if latest_pivot['type'] == 'low':
        ...
        if (current_orderflow['delta_btc'] < thresholds['delta_min'] and
            ...
            return {
                ...
                'forming': True,
                'pivot': latest_pivot,
                'reason': f"Heavy selling ({current_orderflow['delta_btc']:.1f} BTC) absorbed at pivot low. CVD: {features['cvd']:.1f}",
                'confidence': 'HIGH' if features['absorption_score'] > 70 else 'MEDIUM'
            }
    ...
    # BEARISH OB forming
        ...
        if (current_orderflow['delta_btc'] > thresholds['delta_min'] and
            ...
            return {
                ...
                'forming': True,
                'pivot': latest_pivot,
                'reason': f"Heavy buying ({current_orderflow['delta_btc']:.1f} BTC) absorbed at pivot high. CVD: {features['cvd']:.1f}",
                'confidence': 'HIGH' if features['absorption_score'] > 70 else 'MEDIUM'
            }
    ...
    try:
        ...
        with open(SIGNAL_CSV_PATH, 'a', newline='') as f:
            fieldnames = ['timestamp', 'event_type', 'direction', 'tier', 'price', 
                         'delta_btc', 'volume_ratio', 'delta_severity', 'cvd', 
                         'bid_ask_ratio', 'absorption_score', 'large_orders', 'extra']
            ...
            writer.writerow({
                ...
                'absorption_score': features.get('absorption_score', 0),
                'large_orders': features.get('large_orders_count', 0),
...
def send_market_update():
    print("\n" + "="*80)
    print("🔍 SCANNING MARKET WITH FULL ORDER FLOW...")
    print("="*80)
    
    # Fetch comprehensive order flow
...
<b>📊 ORDER FLOW DETAILS:</b>
Delta: {current_orderflow['delta_btc']:.2f} BTC
CVD: {features['cvd']:.2f}
Absorption Score: {features['absorption_score']:.0f}/100
Bid/Ask Ratio: {features['bid_ask_ratio']:.2f}
Large Orders: {features['large_orders_count']}
...
<b>WHAT TO DO:</b>
{"✅ Consider LONG - Strong absorption at support" if ob_forming['type'] == 'bullish' else "✅ Consider SHORT - Strong absorption at resistance"}

<b>⚠️ Note:</b> EARLY signal - use tight stops!
"""
            ...
            log_signal_csv(
                ...
                features=features,
                extra={'confidence': ob_forming['confidence'], 'absorption': features['absorption_score']}
            )
...
def home():
    return f"""
    <h1>🚀 Beast Mode V10 - FULL ORDER FLOW EDITION</h1>
    ...
    <ul>
        ...
        <li>Bid/Ask imbalance scoring</li>
        <li>Absorption metrics at key levels</li>
        <li>Multi-exchange order book aggregation</li>
    </ul>
...
if __name__ == '__main__':
    print("🚀 Beast Mode V10 - FULL ORDER FLOW EDITION")
    print("="*80)
    ...
    if RUN_SCANNER:
        ...
        if SEND_STARTUP_TELEGRAM:
            send_telegram(f"""
🚀 <b>Beast Mode V10 - FULL ORDER FLOW Started!</b>
...
<b>⚡ WHAT YOU'LL SEE NOW:</b>
• Actual aggressive buy/sell flow
• Real-time CVD tracking
• Whale activity alerts
• Order book imbalances
• Delta-price divergences
• Professional absorption metrics

No more 0.00 deltas! Full institutional-grade data! 🔥
