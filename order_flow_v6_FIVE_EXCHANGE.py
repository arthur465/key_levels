...
def get_bitget_orderbook():
    ...
    params = {"symbol": "BTCUSDT_UMCBL", "limit": "150"}
    try:
        print(f"   🟢 [BITGET] Calling orderbook API...")
        response = requests.get(url, params=params, timeout=10)
...
def get_bitget_trades():
    ...
    params = {"symbol": "BTCUSDT_UMCBL", "limit": "500"}
    try:
        print(f"   🟢 [BITGET] Calling trades API...")
        response = requests.get(url, params=params, timeout=10)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                bid_volume = sum(float(b["vol"]) for b in bids)
                ask_volume = sum(float(a["vol"]) for a in asks)
                total_volume = bid_volume + ask_volume
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                buy_volume = sum(float(t["vol"]) for t in trades if t["side"] == 1)  # 1 = buy
                sell_volume = sum(float(t["vol"]) for t in trades if t["side"] == 2)  # 2 = sell
                total = buy_volume + sell_volume
...
def get_gateio_trades():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            buy_volume = sum(float(t["size"]) for t in trades if t["size"] > 0)
            sell_volume = sum(abs(float(t["size"])) for t in trades if t["size"] < 0)
            total = buy_volume + sell_volume
