...
def detect_market_patterns(current_of):
    ...
    # ============= MULTI-TIMEFRAME DIVERGENCE DETECTION =============
    # Check multiple timeframes: 1H, 2H, 4H (active) + 8H (context)
    ...
    if bearish_divs:
        # Count active timeframes (not including 8H context)
        active_tfs = [tf for tf in ['1H', '2H', '4H'] if tf in bearish_divs]
        
        # Only alert if at least one active timeframe triggers
        if active_tfs:
            ...
            if now - last_alert_time > PATTERN_ALERT_COOLDOWN:
                # Calculate average strength from active timeframes
                avg_strength = sum(bearish_divs[tf]['strength'] for tf in active_tfs) / len(active_tfs)
                
                # Confidence boost for multiple timeframes
                ...
                # Build timeframe status
                tf_status = ""
                for tf in ['1H', '2H', '4H']:
                    if tf in bearish_divs:
                        decline = bearish_divs[tf]['decline_pct']
                        tf_status += f"⚡ {tf}: ✅ CONFIRMED (-{decline:.1f}% delta)\n"
                    else:
                        tf_status += f"⚡ {tf}: ❌ Not detected\n"
                
                # Macro context (8H)
                macro_context = ""
                if '8H' in bearish_divs:
                    decline_8h = bearish_divs['8H']['decline_pct']
                    macro_context = f"""
<b>🔷 MACRO CONTEXT:</b>
8H Divergence: ✅ CONFIRMED (-{decline_8h:.1f}%)
Daily Bias: BEARISH (sellers building)

<b>💪 MULTI-TIMEFRAME ALIGNMENT!</b>
{tf_count} active timeframes + 8H macro = ULTRA HIGH CONVICTION
This is a MAJOR top forming, not just a quick reversal!
"""
                else:
                    # Check if 8H is forming (70%+ of the way)
                    if len(orderflow_history) >= int(DIVERGENCE_LOOKBACK_8H * 0.7):
                        recent_8h = list(orderflow_history)[-int(DIVERGENCE_LOOKBACK_8H * 0.7):]
                        prices_8h = [h['price'] for h in recent_8h]
                        deltas_8h = [h['delta'] for h in recent_8h]
                        
                        if prices_8h[-1] > max(prices_8h[:-1]) and deltas_8h[-1] < max(deltas_8h[:-1]):
                            completion = int((len(recent_8h) / DIVERGENCE_LOOKBACK_8H) * 100)
                            macro_context = f"""
<b>🔷 MACRO CONTEXT:</b>
8H Divergence: ⚠️ FORMING ({completion}% complete)
Daily Bias: Turning BEARISH
"""
                
                # Conviction level based on timeframes
                if tf_count == 3:
...
{tf_status}
Aligned: {tf_count}/3 trading timeframes
{macro_context}
<b>📊 WHAT'S HAPPENING:</b>
Price: HIGHER_HIGHS at ${current_price:,.0f}
Delta: WEAKENING across timeframes
  ↳ Current Δ: {format_delta_explanation(current_of['delta'])}
  ↳ Buyers losing strength at higher prices

<b>⚠️ WHAT THIS MEANS:</b>
Price pushing to new highs BUT buying declining
{tf_count} timeframe{"s" if tf_count > 1 else ""} showing weakness = {"MAJOR" if tf_count >= 2 else "Potential"} TOP
Buyers exhausting while sellers prepare
= {"High" if tf_count >= 2 else "Moderate"} probability of REVERSAL DOWN
...
<b>⚡ BIAS: {"STRONGLY " if tf_count >= 2 else ""}BEARISH</b>
{"Major reversal setup" if tf_count >= 2 else "Potential reversal"} from ${current_price:,.0f}

<b>🗣️ IN PLAIN ENGLISH:</b>
Price at new highs but buyers getting weaker on {tf_count} timeframe{"s" if tf_count > 1 else ""}.
{"This is a major top - sellers taking over!" if tf_count >= 2 else "Watch for sellers to take over."}
{"Size up - strong setup!" if tf_count >= 2 else "Standard divergence play."}
                """
    ...
    if bullish_divs:
        ...
        if active_tfs:
            ...
            if now - last_alert_time > PATTERN_ALERT_COOLDOWN:
                ...
                tf_status = ""
                for tf in ['1H', '2H', '4H']:
                    if tf in bullish_divs:
                        improve = bullish_divs[tf]['improve_pct']
                        tf_status += f"⚡ {tf}: ✅ CONFIRMED (+{improve:.1f}% delta)\n"
                    else:
                        tf_status += f"⚡ {tf}: ❌ Not detected\n"
                
                # Macro context (8H)
                macro_context = ""
                if '8H' in bullish_divs:
                    improve_8h = bullish_divs['8H']['improve_pct']
                    macro_context = f"""
<b>🔷 MACRO CONTEXT:</b>
8H Divergence: ✅ CONFIRMED (+{improve_8h:.1f}%)
Daily Bias: BULLISH (buyers building)

<b>💪 MULTI-TIMEFRAME ALIGNMENT!</b>
{tf_count} active timeframes + 8H macro = ULTRA HIGH CONVICTION
This is a MAJOR bottom forming, not just a quick bounce!
"""
                else:
                    # Check if 8H is forming
                    if len(orderflow_history) >= int(DIVERGENCE_LOOKBACK_8H * 0.7):
                        recent_8h = list(orderflow_history)[-int(DIVERGENCE_LOOKBACK_8H * 0.7):]
                        prices_8h = [h['price'] for h in recent_8h]
                        deltas_8h = [h['delta'] for h in recent_8h]
                        
                        if prices_8h[-1] < min(prices_8h[:-1]) and deltas_8h[-1] > min(deltas_8h[:-1]):
                            completion = int((len(recent_8h) / DIVERGENCE_LOOKBACK_8H) * 100)
                            macro_context = f"""
<b>🔷 MACRO CONTEXT:</b>
8H Divergence: ⚠️ FORMING ({completion}% complete)
Daily Bias: Turning BULLISH
"""
...
{macro_context}
<b>📊 WHAT'S HAPPENING:</b>
Price: LOWER_LOWS at ${current_price:,.0f}
Delta: STRENGTHENING across timeframes
  ↳ Current Δ: {format_delta_explanation(current_of['delta'])}
  ↳ Buyers stepping in at lower prices

<b>⚠️ WHAT THIS MEANS:</b>
Price dropping to new lows BUT buying increasing
{tf_count} timeframe{"s" if tf_count > 1 else ""} showing strength = {"MAJOR" if tf_count >= 2 else "Potential"} BOTTOM
Sellers exhausting while buyers accumulate
= {"High" if tf_count >= 2 else "Moderate"} probability of REVERSAL UP
...
<b>⚡ BIAS: {"STRONGLY " if tf_count >= 2 else ""}BULLISH</b>
{"Major reversal setup" if tf_count >= 2 else "Potential reversal"} from ${current_price:,.0f}

<b>🗣️ IN PLAIN ENGLISH:</b>
Price at new lows but buyers getting stronger on {tf_count} timeframe{"s" if tf_count > 1 else ""}.
{"This is a major bottom - buyers taking over!" if tf_count >= 2 else "Watch for buyers to take over."}
{"Size up - strong setup!" if tf_count >= 2 else "Standard divergence play."}
                """
...
def home():
    ...
    <h3>🆕 MULTI-TIMEFRAME DIVERGENCES:</h3>
    <ul>
        <li>⚡ 1H Divergence - Scalp plays (4 readings)</li>
        <li>⚡ 2H Divergence - Day trades (8 readings)</li>
        <li>⚡ 4H Divergence - Swing setups (16 readings)</li>
        <li>🔷 8H Context - Macro bias (32 readings)</li>
    </ul>
    
    <h3>NEW FEATURES:</h3>
    <ul>
        <li>✅ Multi-Timeframe Divergence Detection</li>
        <li>✅ Confidence Boost When Aligned</li>
        <li>✅ 8H Macro Context (not standalone alerts)</li>
...
if __name__ == '__main__':
    ...
    print("  ⚡ 1H - Scalp plays (4 readings)")
    print("  ⚡ 2H - Day trades (8 readings)")
    print("  ⚡ 4H - Swing setups (16 readings)")
    print("  🔷 8H - Macro context (32 readings)")
    print("="*80)
    print("NEW FEATURES:")
    print("  ✅ Multi-Timeframe Divergence Detection")
    print("  ✅ Confidence Boost When Timeframes Align")
    print("  ✅ 8H Macro Context (shown in alerts)")
...
⚡ 1H, 2H, 4H (active trading)
🔷 8H (macro context only)

<b>When 2-3 timeframes align = ULTRA HIGH CONVICTION! 🔥</b>

<b>NEW FEATURES:</b>
✅ Multi-Timeframe Detection
✅ Confidence Boost (aligned TFs)
✅ 8H Macro Context
