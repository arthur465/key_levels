...
# Alert focus controls to reduce spam and prioritize accumulation/distribution setups.
ALERT_FOCUS_MODE = os.environ.get('ALERT_FOCUS_MODE', 'absorption_only').lower()  # absorption_only or all
ABSORPTION_MIN_RATIO = float(os.environ.get('ABSORPTION_MIN_RATIO', '2.2'))
ABSORPTION_MAX_PRICE_CHANGE_PCT = float(os.environ.get('ABSORPTION_MAX_PRICE_CHANGE_PCT', '0.12'))
ABSORPTION_PERSISTENCE_BARS = int(os.environ.get('ABSORPTION_PERSISTENCE_BARS', '2'))
ABSORPTION_COOLDOWN_SECONDS = int(os.environ.get('ABSORPTION_COOLDOWN_SECONDS', '10800'))
ENABLE_QUALITY_FILTER = os.environ.get('ENABLE_QUALITY_FILTER', '1') == '1'
MIN_QUALITY_SCORE = float(os.environ.get('MIN_QUALITY_SCORE', '6.5'))
ENABLE_OUTCOME_LOG = os.environ.get('ENABLE_OUTCOME_LOG', '1') == '1'
OUTCOME_LOG_PATH = os.environ.get('OUTCOME_LOG_PATH', 'absorption_outcomes.csv')
OUTCOME_EVAL_BARS = int(os.environ.get('OUTCOME_EVAL_BARS', '4'))
OUTCOME_MOVE_THRESHOLD_PCT = float(os.environ.get('OUTCOME_MOVE_THRESHOLD_PCT', '0.35'))
...
last_pattern_alerts = {}
absorption_streaks = {'demand': 0, 'supply': 0}
pending_absorption_outcomes = []
...
def build_absorption_entry_playbook(direction, current_price, structure, delta_ratio):
    """
    2-tier entry plan for 30m day-trader execution.
    direction: 'bullish' for demand absorption, 'bearish' for supply absorption
    """
    if direction == 'bullish':
        structure_break = structure.get('resistance') or (current_price * 1.002)
        invalidation = structure.get('support') or (current_price * 0.996)
        probe_text = (
            "Tier 1 (Probe 25-35%): Enter only if next scan shows delta improving "
            "(less negative or flipping positive) while price holds this zone."
        )
        add_text = (
            f"Tier 2 (Add 65-75%): Add only after 30m bullish structure break above "
            f"${structure_break:,.2f}."
        )
        invalid_text = (
            f"Invalidation: Exit if price loses ${invalidation:,.2f} or delta re-accelerates "
            "negative for 2 scans."
        )
    else:
        structure_break = structure.get('support') or (current_price * 0.998)
        invalidation = structure.get('resistance') or (current_price * 1.004)
        probe_text = (
            "Tier 1 (Probe 25-35%): Enter only if next scan shows delta weakening "
            "(less positive or flipping negative) while price stalls here."
        )
        add_text = (
            f"Tier 2 (Add 65-75%): Add only after 30m bearish structure break below "
            f"${structure_break:,.2f}."
        )
        invalid_text = (
            f"Invalidation: Exit if price reclaims ${invalidation:,.2f} or delta re-accelerates "
            "positive for 2 scans."
        )

    return (
        "\n<b>🧭 DAY-TRADER EXECUTION (30m):</b>\n"
        f"{probe_text}\n"
        f"{add_text}\n"
        f"{invalid_text}\n"
        "Time-stop: If no structure break within 2-3 scans (30-45 min), reduce/exit probe.\n"
        f"Signal quality: {delta_ratio:.1f}x flow imbalance."
    )


def get_entry_trigger_status(direction, recent_history, structure):
    """
    Returns one of: NOT READY, PROBE READY, ADD READY
    direction: 'bullish' or 'bearish'
    """
    if len(recent_history) < 3:
        return "NOT READY", "Need more scans to validate trigger progression."

    d0 = recent_history[-3]['delta']
    d1 = recent_history[-2]['delta']
    d2 = recent_history[-1]['delta']
    p1 = recent_history[-2]['price']
    p2 = recent_history[-1]['price']
    price_change_pct = ((p2 - p1) / p1 * 100) if p1 else 0

    if direction == 'bullish':
        delta_improving = abs(d2) < abs(d1) or d2 > 0
        structure_break = structure.get('resistance')
        broke_structure = bool(structure_break and p2 > structure_break)
        if broke_structure:
            return "ADD READY", f"30m structure break confirmed above ${structure_break:,.2f}."
        if delta_improving and d2 > d0:
            return "PROBE READY", "Delta is improving and demand absorption is stabilizing."
        return "NOT READY", "Waiting for delta improvement or bullish structure break."
    else:
        delta_weakening = abs(d2) < abs(d1) or d2 < 0
        structure_break = structure.get('support')
        broke_structure = bool(structure_break and p2 < structure_break)
        if broke_structure:
            return "ADD READY", f"30m structure break confirmed below ${structure_break:,.2f}."
        if delta_weakening and d2 < d0:
            return "PROBE READY", "Delta is weakening and supply absorption is stabilizing."
        return "NOT READY", "Waiting for delta weakening or bearish structure break."


def format_status_for_telegram(status, direction):
    if status == "ADD READY":
        icon = "🟢"
        if direction == 'bullish':
            action = "Action Now: Add primary size (65-75%) on confirmed 30m break."
        else:
            action = "Action Now: Add primary size (65-75%) on confirmed 30m break."
    elif status == "PROBE READY":
        icon = "🟡"
        if direction == 'bullish':
            action = "Action Now: Probe only (25-35%), wait for 30m break before full size."
        else:
            action = "Action Now: Probe only (25-35%), wait for 30m break before full size."
    else:
        icon = "🔴"
        action = "Action Now: No entry yet; monitor next scan for trigger."

    return f"{icon} {status}", action


def compute_absorption_quality_score(direction, delta_ratio, persistence, structure, nearby_levels):
    """
    Score 0-10 for filtering noisy absorption alerts.
    """
    score = 0.0
    score += min(3.5, max(0.0, (delta_ratio - 1.5) * 1.5))  # flow imbalance strength
    score += min(2.0, persistence * 0.8)                     # persistence confidence
    score += min(1.5, len(nearby_levels) * 0.6)              # level proximity
...
def register_absorption_outcome(direction, entry_price, score, delta_ratio, persistence):
    pending_absorption_outcomes.append({
        'direction': direction,
        'entry_price': entry_price,
        'score': score,
        'delta_ratio': delta_ratio,
        'persistence': persistence,
        'entry_index': len(orderflow_history) - 1,
        'entry_time': datetime.now(timezone.utc)
    })


def evaluate_absorption_outcomes():
    if not ENABLE_OUTCOME_LOG or not pending_absorption_outcomes or len(orderflow_history) < 2:
        return

    latest_price = orderflow_history[-1]['price']
    rows_to_write = []
    for setup in list(pending_absorption_outcomes):
        bars_elapsed = (len(orderflow_history) - 1) - setup['entry_index']
        if bars_elapsed < OUTCOME_EVAL_BARS:
            continue

        move_pct = ((latest_price - setup['entry_price']) / setup['entry_price']) * 100
        if setup['direction'] == 'bullish':
            success = move_pct >= OUTCOME_MOVE_THRESHOLD_PCT
        else:
            success = move_pct <= -OUTCOME_MOVE_THRESHOLD_PCT

        rows_to_write.append({
            'entry_time_utc': setup['entry_time'].isoformat(),
            'direction': setup['direction'],
            'entry_price': round(setup['entry_price'], 2),
            'eval_price': round(latest_price, 2),
            'bars_elapsed': bars_elapsed,
            'move_pct': round(move_pct, 4),
            'score': setup['score'],
            'delta_ratio': round(setup['delta_ratio'], 4),
            'persistence': setup['persistence'],
            'success': int(success)
        })
        pending_absorption_outcomes.remove(setup)

    if not rows_to_write:
        return

    try:
        file_exists = os.path.exists(OUTCOME_LOG_PATH)
        with open(OUTCOME_LOG_PATH, mode='a', newline='', encoding='utf-8') as f:
...
def detect_market_patterns(current_of):
    ...
    # ============= MULTI-TIMEFRAME DIVERGENCE DETECTION =============
    enable_non_absorption_alerts = ALERT_FOCUS_MODE == 'all'
    timeframes = {
    ...
    if enable_non_absorption_alerts:
    ...
    if enable_non_absorption_alerts and bearish_divs:
...
"""
    ...
    if enable_non_absorption_alerts and bullish_divs:
...
"""
    ...
    if len(orderflow_history) >= 3:
        ...
        recent_window = list(orderflow_history)[-min(20, len(orderflow_history)):]
        avg_delta = sum(abs(h['delta']) for h in recent_window) / len(recent_window) if recent_window else 0
        price_change = ((recent[-1]['price'] - recent[-2]['price']) / recent[-2]['price']) * 100
        delta_ratio = abs(current_delta) / avg_delta if avg_delta else 0
        
        # Demand absorption: Heavy selling with minimal price drop
        if current_delta < 0 and delta_ratio >= ABSORPTION_MIN_RATIO and abs(price_change) <= ABSORPTION_MAX_PRICE_CHANGE_PCT:
            absorption_streaks['demand'] += 1
            absorption_streaks['supply'] = 0
            severity = min(10.0, delta_ratio * 3)
            
            cooldown_key = 'demand_absorption'
            last_alert_time = last_pattern_alerts.get(cooldown_key, 0)
            
            if absorption_streaks['demand'] >= ABSORPTION_PERSISTENCE_BARS and now - last_alert_time > ABSORPTION_COOLDOWN_SECONDS:
                ...
                structure = get_structure_context()
                nearby_levels = check_key_level_proximity(current_price)
                quality_score = compute_absorption_quality_score(
                    direction='bullish',
                    delta_ratio=delta_ratio,
                    persistence=absorption_streaks['demand'],
                    structure=structure,
                    nearby_levels=nearby_levels
                )
                if ENABLE_QUALITY_FILTER and quality_score < MIN_QUALITY_SCORE:
                    print(
                        f"   ⏭️ Skipping demand absorption alert (quality {quality_score}/10 < {MIN_QUALITY_SCORE})"
                    )
                else:
                    trigger_status, trigger_reason = get_entry_trigger_status(
                        direction='bullish',
                        recent_history=list(orderflow_history)[-5:],
                        structure=structure
                    )
                    trigger_label, action_now = format_status_for_telegram(trigger_status, 'bullish')
                    playbook = build_absorption_entry_playbook(
                        direction='bullish',
                        current_price=current_price,
                        structure=structure,
                        delta_ratio=delta_ratio
...
↳ Current is {delta_ratio:.1f}x heavier
...
Persistence: {absorption_streaks['demand']}/{ABSORPTION_PERSISTENCE_BARS}
Quality Score: {quality_score:.2f}/10

<b>🚦 ENTRY TRIGGER STATUS: {trigger_label}</b>
{trigger_reason}
<b>⚡ {action_now}</b>
{playbook}
{perf_info}
...
"""
        ...
        elif current_delta > 0 and delta_ratio >= ABSORPTION_MIN_RATIO and abs(price_change) <= ABSORPTION_MAX_PRICE_CHANGE_PCT:
            absorption_streaks['supply'] += 1
            absorption_streaks['demand'] = 0
            severity = min(10.0, delta_ratio * 3)
            
            cooldown_key = 'supply_absorption'
            last_alert_time = last_pattern_alerts.get(cooldown_key, 0)
            
            if absorption_streaks['supply'] >= ABSORPTION_PERSISTENCE_BARS and now - last_alert_time > ABSORPTION_COOLDOWN_SECONDS:
                add_to_memory('absorption', {
                    'type': 'supply',
                    'price': current_price,
                    'delta': current_delta,
                ...
                quality_score = compute_absorption_quality_score(
                    direction='bearish',
                    delta_ratio=delta_ratio,
                    persistence=absorption_streaks['supply'],
                    structure=structure,
                    nearby_levels=nearby_levels
                )
                if ENABLE_QUALITY_FILTER and quality_score < MIN_QUALITY_SCORE:
                    print(
                        f"   ⏭️ Skipping supply absorption alert (quality {quality_score}/10 < {MIN_QUALITY_SCORE})"
                    )
                else:
                    trigger_status, trigger_reason = get_entry_trigger_status(
                        direction='bearish',
                    ...
                    trigger_label, action_now = format_status_for_telegram(trigger_status, 'bearish')
                    playbook = build_absorption_entry_playbook(
                        direction='bearish',
                        current_price=current_price,
                        structure=structure,
...
Persistence: {absorption_streaks['supply']}/{ABSORPTION_PERSISTENCE_BARS}
Quality Score: {quality_score:.2f}/10
...
"""
            ...
            # Reset streak when no valid absorption pattern is present.
            absorption_streaks['demand'] = 0
            absorption_streaks['supply'] = 0
    
    # ============= CLIMAX PATTERNS =============
    if enable_non_absorption_alerts and len(orderflow_history) >= 5:
...
if __name__ == '__main__':
    ...
    print("="*80)
    print("🎯 ALERT FOCUS:")
    print(f"  Mode: {ALERT_FOCUS_MODE}")
    print(f"  Absorption ratio min: {ABSORPTION_MIN_RATIO:.1f}x")
    print(f"  Absorption persistence: {ABSORPTION_PERSISTENCE_BARS} scans")
    print(f"  Absorption cooldown: {ABSORPTION_COOLDOWN_SECONDS//60} minutes")
    print(f"  Quality filter: {ENABLE_QUALITY_FILTER} (min {MIN_QUALITY_SCORE:.1f}/10)")
    print(f"  Outcome logging: {ENABLE_OUTCOME_LOG} ({OUTCOME_LOG_PATH})")
    print("="*80)
