...
"""
                ...
                # Register zone for entry trigger watching
                register_entry_zone(context['pattern'], context['bias'], current_price, context['confidence'], context['reason'])
...
# ============= ENTRY TRIGGER SYSTEM (V9 ADD-ON) =============
#
# HOW IT WORKS:
#   1. Accumulation/Distribution detected → zone is saved (high/low of consolidation)
#   2. Every 15min, check_entry_triggers() scans all active zones
#   3. Entry fires when ALL 3 conditions are met:
#      - Price breaks out of the zone range
#      - Delta confirms the direction (positive for long, negative for short)
#      - CVD momentum is trending the right way
#   4. Alert includes entry price, stop loss, target, and R:R
#
# PAPER TRADING MODE: Set PAPER_TRADING = True to just log signals, no real orders.
# =========================================================================

PAPER_TRADING = True          # Flip to False when you're ready to go live
RISK_REWARD = 2.0             # Target = 2x the stop distance
BREAKOUT_THRESHOLD_PCT = 0.25 # Price must break zone by this % to confirm
ENTRY_COOLDOWN = 3600         # 1 hour between entries for same direction
MIN_DELTA_CONFIRM = 500       # Delta must be at least this strong to confirm entry

# Stores active zones waiting for entry trigger
entry_zones = deque(maxlen=20)
last_entry_alerts = {}        # Cooldown tracker per direction

class EntryZone:
    """Represents a detected accumulation/distribution zone waiting for breakout"""
    def __init__(self, pattern, bias, price, confidence, reason):
        self.pattern   = pattern      # 'accumulation' | 'distribution'
        self.bias      = bias         # 'bullish' | 'bearish'
        ...
        # Build zone range from recent price history
        if len(orderflow_history) >= 3:
            recent_prices = [h['price'] for h in list(orderflow_history)[-5:]]
            self.zone_high = max(recent_prices) * 1.001  # tiny buffer above
            self.zone_low  = min(recent_prices) * 0.999  # tiny buffer below
        else:
            self.zone_high = price * 1.003
            self.zone_low  = price * 0.997

        # Pre-calculate stop and target
        zone_size = self.zone_high - self.zone_low
        if bias == 'bullish':
            self.stop_price   = self.zone_low * 0.999       # just below zone low
            self.target_price = self.zone_high + (zone_size * RISK_REWARD)
        else:
            self.stop_price   = self.zone_high * 1.001      # just above zone high
            self.target_price = self.zone_low - (zone_size * RISK_REWARD)

        self.risk_distance = abs(price - self.stop_price)
        self.reward_distance = abs(self.target_price - price)
        self.rr_ratio = (self.reward_distance / self.risk_distance) if self.risk_distance > 0 else 0

        # Position size (from account settings at top of file)
        risk_amount = ACCOUNT_SIZE * RISK_PER_TRADE
        self.position_size = risk_amount / self.risk_distance if self.risk_distance > 0 else 0

    def is_expired(self):
        """Zone expires after 4 hours if not triggered"""
        age = (datetime.now(timezone.utc) - self.created_at).total_seconds() / 3600
        return age > 4.0

    def __repr__(self):
        return f"EntryZone({self.bias.upper()} {self.pattern} @ ${self.entry_price:,.0f})"


def register_entry_zone(pattern, bias, price, confidence, reason):
    """Called from detect_absorption_patterns() when a pattern is confirmed"""
    zone = EntryZone(pattern, bias, price, confidence, reason)
    entry_zones.append(zone)
    print(f"📍 Entry zone registered: {zone}")
    print(f"   Zone: ${zone.zone_low:,.0f} - ${zone.zone_high:,.0f}")
    print(f"   Stop: ${zone.stop_price:,.0f} | Target: ${zone.target_price:,.0f} | R:R {zone.rr_ratio:.1f}")


def compute_cvd_momentum():
    """
    Calculate CVD trend from recent orderflow history.
    Returns: 'bullish', 'bearish', or 'neutral'
    """
    if len(orderflow_history) < 5:
        return 'neutral'

    recent = list(orderflow_history)[-6:]
    deltas = [h['delta'] for h in recent]

    # Running CVD = cumulative sum of deltas
    cvd = []
    running = 0
    for d in deltas:
        running += d
    ...
    # Trend = is CVD going up or down over last 3 points?
    if cvd[-1] > cvd[-3]:
        return 'bullish'
    elif cvd[-1] < cvd[-3]:
        return 'bearish'
...
def check_entry_triggers():
    """
    Main function — call this every 15min alongside detect_absorption_patterns().
    Scans all active entry zones and fires entry alert if breakout + delta + CVD confirm.
    """
    current_price = get_current_price()
    if not current_price:
        return
    ...
    latest = list(orderflow_history)[-1]
    current_delta = latest.get('delta', 0)
    cvd_trend     = compute_cvd_momentum()
    now           = time.time()

    for zone in list(entry_zones):

        # Skip already used or expired zones
        if zone.triggered or zone.expired:
            continue
        if zone.is_expired():
            zone.expired = True
            print(f"⏰ Zone expired: {zone}")
            continue

        direction    = zone.bias  # 'bullish' or 'bearish'
        cooldown_key = f"entry_{direction}"
        last_alert   = last_entry_alerts.get(cooldown_key, 0)

        if now - last_alert < ENTRY_COOLDOWN:
            continue

        # ---- CHECK LONG ENTRY ----
        if direction == 'bullish':
            breakout_level  = zone.zone_high * (1 + BREAKOUT_THRESHOLD_PCT / 100)
            price_broke_out = current_price >= breakout_level
            delta_confirms  = current_delta >= MIN_DELTA_CONFIRM
            cvd_confirms    = cvd_trend == 'bullish'

            if price_broke_out and delta_confirms and cvd_confirms:
                zone.triggered = True
                last_entry_alerts[cooldown_key] = now
                _fire_entry_alert(zone, current_price, current_delta, cvd_trend, 'LONG')

        # ---- CHECK SHORT ENTRY ----
        elif direction == 'bearish':
            breakout_level  = zone.zone_low * (1 - BREAKOUT_THRESHOLD_PCT / 100)
            price_broke_out = current_price <= breakout_level
            delta_confirms  = current_delta <= -MIN_DELTA_CONFIRM
            cvd_confirms    = cvd_trend == 'bearish'

            if price_broke_out and delta_confirms and cvd_confirms:
                zone.triggered = True
                last_entry_alerts[cooldown_key] = now
                _fire_entry_alert(zone, current_price, current_delta, cvd_trend, 'SHORT')


def _fire_entry_alert(zone, current_price, current_delta, cvd_trend, side):
    """Build and send the entry alert"""
    paper_tag = "📋 PAPER TRADE" if PAPER_TRADING else "🔴 LIVE SIGNAL"
    emoji     = "🟢" if side == 'LONG' else "🔴"

    risk_amount  = ACCOUNT_SIZE * RISK_PER_TRADE
    pos_size_str = f"{zone.position_size:.4f} BTC" if zone.position_size < 1 else f"{zone.position_size:.2f} BTC"

    message = f"""
{emoji} <b>{paper_tag} — {side} ENTRY TRIGGERED</b>
Pattern: {zone.pattern.upper()} ({zone.bias.upper()})
Confidence: {zone.confidence:.1f}/10

<b>📐 TRADE SETUP:</b>
Entry:  ${current_price:,.2f}
Stop:   ${zone.stop_price:,.2f}  ({abs(current_price - zone.stop_price) / current_price * 100:.2f}% away)
Target: ${zone.target_price:,.2f}  ({abs(zone.target_price - current_price) / current_price * 100:.2f}% away)
R:R     {zone.rr_ratio:.1f}:1

<b>💰 POSITION SIZING:</b>
Account:  ${ACCOUNT_SIZE:,}
Risk 2%:  ${risk_amount:,.0f}
Size:     {pos_size_str}

<b>💎 CONFIRMATION SIGNALS:</b>
✅ Price broke zone {('high' if side == 'LONG' else 'low')} (${zone.zone_high:,.0f if side == 'LONG' else zone.zone_low:,.0f})
✅ Delta confirming: {current_delta:+,.0f} BTC
✅ CVD momentum: {cvd_trend.upper()}

<b>🔍 ORIGINAL SIGNAL:</b>
{zone.reason}
Zone detected: {zone.created_at.strftime('%H:%M UTC')}

⚠️ <i>Entry Trigger System v9 — {'PAPER MODE' if PAPER_TRADING else 'LIVE MODE'}</i>
"""

    send_telegram(message)
    print(f"🚀 {'[PAPER]' if PAPER_TRADING else '[LIVE]'} {side} entry fired @ ${current_price:,.2f}")
    print(f"   Stop: ${zone.stop_price:,.2f} | Target: ${zone.target_price:,.2f} | R:R {zone.rr_ratio:.1f}")


# ============= MAIN EXECUTION EXAMPLE =============
def send_startup_message():
    """Send Telegram ping on startup so you know the bot is alive"""
    msg = (
        "🚀 <b>Beast Mode V9 is LIVE</b>\n"
        f"Mode: {'📋 PAPER TRADING' if PAPER_TRADING else '🔴 LIVE TRADING'}\n"
        "Watching: BTCUSDT\n"
        "Interval: 15 minutes\n\n"
        "✅ Entry trigger system armed\n"
        "✅ Accumulation/Distribution detection active\n\n"
        f"<i>Started at {datetime.now(timezone.utc).strftime('%H:%M UTC on %b %d')}</i>"
    )
    send_telegram(msg)
...
def main_loop():
    """Main loop — runs every 15 minutes forever"""
    print("🚀 Beast Mode V9 - ENTRY TRIGGER EDITION")
    print("=" * 60)
    print(f"  Mode: {'📋 PAPER TRADING' if PAPER_TRADING else '🔴 LIVE'}")
    print(f"  Interval: {ORDERFLOW_INTERVAL}s ({ORDERFLOW_INTERVAL // 60} min)")
    print("=" * 60)

    # Ping Telegram so you know it's alive
    send_startup_message()

    while True:
        try:
            now_str = datetime.now(timezone.utc).strftime('%H:%M:%S UTC')
            print(f"\n⏱️  [{now_str}] Running cycle...")

            current_price = get_current_price()
            if current_price:
                print(f"   BTC Price: ${current_price:,.2f}")

                # Step 1: Detect accumulation/distribution zones
                if len(orderflow_history) >= 5:
                    latest = list(orderflow_history)[-1]
                    delta        = latest.get('delta', 0)
                    price_change = latest.get('price_change_pct', 0)
                    detect_absorption_patterns(delta, current_price, price_change)

                    # Step 2: Check if any zone has triggered an entry
                    check_entry_triggers()

                    # Step 3: Check past absorption outcomes (learning)
                    track_absorption_outcome()
                else:
                    print(f"   Warming up... ({len(orderflow_history)}/{5} candles needed)")
            else:
                print("   ⚠️  Could not fetch price — skipping cycle")

        except Exception as e:
            print(f"   ❌ Error in main loop: {e}")
            # Don't crash — just log and continue on next cycle

        print(f"   💤 Sleeping {ORDERFLOW_INTERVAL // 60} min...")
