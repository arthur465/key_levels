    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                # FIXED: side is integer: 1=open long (buy), 2=close short (buy), 3=close long (sell), 4=open short (sell)
                # Simplified: 1,2 = buy, 3,4 = sell
                buy_volume = sum(float(t["vol"]) for t in trades if t["side"] in [1, 2])
                sell_volume = sum(float(t["vol"]) for t in trades if t["side"] in [3, 4])
                total = buy_volume + sell_volume
