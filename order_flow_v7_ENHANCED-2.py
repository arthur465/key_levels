...
def get_mexc_trades():
    """MEXC trades - ENHANCED with detailed logging"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                print(f"   🟡 [MEXC] Got {len(trades)} trades, examining first trade...")
                
                # Debug: Print first trade to see structure
                if trades:
                    first_trade = trades[0]
                    print(f"   🟡 [MEXC] First trade keys: {list(first_trade.keys())}")
                    print(f"   🟡 [MEXC] First trade sample: {first_trade}")
                ...
                for t in trades:
                    try:
                        ...
                        # Match against different formats
                        is_buy = False
                        is_sell = False
                        
                        if side in [1, "1", "buy", "BUY", "Buy"]:
                            is_buy = True
                        elif side in [2, "2", "sell", "SELL", "Sell"]:
                            is_sell = True
                ...
                print(f"   🟡 [MEXC] Raw volumes - Buy: {buy_volume}, Sell: {sell_volume}")
